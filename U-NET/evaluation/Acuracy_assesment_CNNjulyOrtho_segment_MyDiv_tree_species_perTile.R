...
#read the orthoimage
orthodir <-   "/mnt/gsdata/projects/bigplantsens/2_UNET_on_Flora_Mask/01_MyDiv/Data/UAV_Orthoimags/Ortho_July/"
...
#load predictions
#path_pred <- "/mnt/gsdata/users/soltani/Workshop_home_fromSSD2/Workshop_home/2_Unet_on_flora_mask/2_MyDiv/Pred_outputUnet_OrthoJuly_flora_mask_filteronly_under15_stem_30percentcover_4kdata_class12_class10_ignored_img1024_50/Majority_vote/"
...
if(length(predictions_list)==20){
  ...
  #read all plots
  AOI <- mixedsort(list.files(orthodir,pattern = "Plot_boundary.shp",recursive = TRUE,full.names = TRUE)) %>%readOGR()#st_read() %>% 
    #filter(Ortho2_inc=="1") %>% as("Spatial")
}


# #detect
cores <- 20#detectCores()-8
...
Acc_val <- foreach(g = seq_len(nrow(AOI)), .packages = c("sf","terra","raster", "rgdal","tidyverse","MLmetrics", "rgeos","foreach","doParallel"), .inorder = T) %dopar% { #,.combine = rbind
  #   
  # accuracy_list <- list()
  # for(g in 1:nrow(AOI)){
