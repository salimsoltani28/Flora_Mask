...
gpu_index <- 3 # Change this to the desired GPU index (e.g., 1, 2, etc.)
...
Ignored_class= 10L #customized loass that ignor the selected class
...
source("scripts/utils/Focal_Tversky_loss_after_paper_implementation.R")
...
all_imags_filtered <- data_path_metadata %>%
  ...
  #filter(!ref==11) %>%
    filter(stam_nostam==0 ,dist<20, pixel_percent>30) %>% dplyr::select(img, mask,ref) #%>%
...
table(data_path_metadata$ref)
table(all_imags_filtered$ref)

#model name
model_dir_name <- "EXP3.1_ov0.2_under20_stem_30percent_class11_Ortho_backRplcd_ZMExcld_img512_FTky_loss0.3_0.7_0.75_CLS10IGNRD"
...
source("scripts/utils/Unet_model_ReLu_Optimized.R")
#with(strategy$scope(), {
model <- get_unet_128()
#}))
...
#garss_class_label= garss_class_label,
...
cp_callback <- callback_model_checkpoint(filepath = filepath,
                                         ...
                                         save_best_only = TRUE,
                                         verbose = 0, #How often print the model update. 1: very often and 0: not often
                                         #mode = "auto",
                                         mode = "max",
                                         save_freq = "epoch")
...
# # Define the function to load the best model based on the highest epoch
...
# code_artifact$add_file("/mnt/gsdata/projects/bigplantsens/1_Flora_mask/01_MyDiv/scripts/01_Training_Unet_Flora_mask_Optimized.R")  # Replace with the actual path
